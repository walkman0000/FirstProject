...
uint8 testF=0;
uint16 secondTwo=0;
uint16 Msencond300 ,mainDelay_1mS_Count ;
uint16 mainDelay_1S_Count,mainDelay_10S_Count;
/****************************************************************************
* 名称：IRQ_Time0()
* 功能：中断Time0服务函数
* 入口参数：无
...
{ 
  ...
  //T1_check();
  if(mainDelay_1mS_Count<=9)mainDelay_1mS_Count++;
...
{ 							   
   ...
   T1MR0 = 1659;			   // 设置MR0比较值(us级定时值) 定时 1 ms ，用于判断上位机超时、传感器超时、上位机延时发送
   T1TCR |= 0x00;  
