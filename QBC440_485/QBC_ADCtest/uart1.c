...
#define T1_PullBusDelay 100
#define T1_sendOverDelayTime 150 //单位ms
uint8 T1_StepSendFlag/*发送下一位的标志*/,T1_StepSendNum=0/*写数据的阶段代码*/,T1_WriteSequence/*写类型的顺序*/;
//T1定义
uint8 T1_Rec_Buffer[50],T1_Send_Buffer[130],T1_Send_Length, T1_sendCount=0,T1_sendIndex=0,T1_haveRecData=0,T1_recCount=0,T1_PermitRecFlag=0;
//T1定义
...
void T1_check(void)
{
if(T1_YlToJOS2_CalHeartBeatValue<20000)T1_YlToJOS2_CalHeartBeatValue++;
  else{
  //地址1的通讯监控
 ...
 {
  if(T1_sendOverTimeValue<T1_sendOverDelayTime*10)T1_sendOverTimeValue++; //等待的超时时间
  else{
...
if(T1_beforeSendCount==1)
  {
   T1_beforeSendValue++;
   if(T1_beforeSendValue>T1_sendDelay*10)
   {
 ...
 if(T1_beforePullBusCount==1)
  {
   ...
   if(T1_beforePullBusValue>T1_PullBusDelay*10) //发送之前等待的时间
   {
 ...
 if(T1_ItsRecTime==1)
 { 
      if(T1_receive_time<55)
	  {
	   T1_receive_time++;
	  }
	  
	  if((T1_receive_time>50)&&(T1_haveRecData==1))	 //波特率9600超时为4ms
	  {	
...
{  uint16 i,startAddress,readCount,writeCount,tmpH,tmpL;
    ...
    {if((T1_Rec_Buffer[0]==QBCAddress))
	 ...
	 {
	   startAddress = (T1_Rec_Buffer[2]<<8)+T1_Rec_Buffer[3];
	   writeCount = (T1_Rec_Buffer[4]<<8)+T1_Rec_Buffer[5];
	   
	   if((startAddress >= 0x80)&&(writeCount<=9)&&((startAddress + writeCount -1)<=0x88))
	   {
	   	  for(i=0;i<writeCount;i++)
		  {
		   QBCWriteBuff[i+startAddress-0x80]=(T1_Rec_Buffer[i*2+7]<<8)+T1_Rec_Buffer[i*2+8];
		  }	
		    T1_sendCount= 8;
			T1_Send_Buffer[0]=QBCAddress;
			T1_Send_Buffer[1]=0x10;
			T1_Send_Buffer[2]=startAddress>>8;	//写的起始地址41049 =0x0418
			T1_Send_Buffer[3]=startAddress;
			T1_Send_Buffer[4]=writeCount>>8;
			T1_Send_Buffer[5]=writeCount;
