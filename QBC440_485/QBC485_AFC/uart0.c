...
{ //监控与三个联力开关的通讯状态
 ...
 {
  if(T0_sendOverTimeValue<T0_sendOverDelayTime*10)T0_sendOverTimeValue++; //等待的超时时间
  else{
 ...
 if(T0_haveRecDone == 1)
{
	...
	SendNextFlag=1;	//接收判断完成，可以发送下一帧
}
//如果通讯状态不正常，则清零接收缓冲区。
//如果3500A/1140通讯正常 ---10回路	如果不正常则需要断所有刮板机电机
...
//如果3300通讯正常	---6回路  如果不正常则需要断转载电机和破碎电机
  ...
  if(SendNextFlag==1)
   {
    //Tx0_EN(1);   //允许发
	//T0_beforeSendCount=1;
	T0_beforePullBusCount=1; //延时一段时间再拉总线
    SendNextFlag=0;
	if(SendSameTimes>=T0_SameFrameSendCount)//如果同一地址同一帧已经发了三次
	{
	 T0_SendFrameNum++;
	  SendSameTimes=0;
	...
	//此处把T0_SendFrameNum当做同一地址查询数据时发送的第几帧的标识
	switch(T0_SendFrameNum)
	{
	   case 1:
